function ClearNights.level_up_table_tailends(card, hands_upgraded, description, instant, level_up, chip_upgrade, mult_upgrade, money_upgrade, juice_cards)
  ...
  --upgrade
  ClearNights.level_up_table(card, hands_upgraded, description, instant, level_up, chip_upgrade, mult_upgrade, money_upgrade, juice_cards)
...
function ClearNights.level_up_table(card, hands_upgraded, description, instant, level_up, chip_upgrade, mult_upgrade, money_upgrade, juice_cards)
  --set vars
  ...
  for i = 1, #hands_upgraded do
    ...
    hand.CN_money = (hand.CN_money or 0) + money_upgrade
    
    --alter game info page if needed
    if hand.CN_money ~= 0 then 
      G.GAME.CN_money_bonus_given = true
    end
  ...
  if not instant then
    ...
    local function shakeit_money(money_text, card, juice_secondary, stat_type, tarot_pulse)
      attention_text({ 
        text = money_text, 
        scale = 0.8, 
        hold = 1, 
